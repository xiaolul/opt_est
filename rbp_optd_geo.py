...
class RBPOptDoc (threading.Thread):
    ...
    rbp_optd.py
    Separate the file only for running purpose.
    """
    ...
    def _est_func_geo_perdoc(self, w):
        """
        ...
        for i in range(0, self._runnum):
            ...
            for j in range(0, self._k):
                tmp_bg = curr_rank[j, :]
                tmp_bg[tmp_bg == 0] = 10 ** -6
                tmp_bg = np.log(tmp_bg)
                tmp_val +=  (np.power(self._p, j) * np.exp(np.dot(w, tmp_bg)) - curr_rbp[j]) ** 2
    ...
    def get_weight_perdoc(self):
        ...
        #
        res = minimize(self._est_func_geo_perdoc, w0, method='SLSQP', constraints=cons, bounds=bnds)
        res_str = str(self._q)
...
def main(argv):
    ...
    out_dir = prefix_dir + collection + "/background_gain/opt_doc_geo/" + str(depth) + "/"
    #
